...
def update_avatar(request):
    ...
    serializer = AvatarS(request.user, data=request.FILES, partial=True, context={'request': request})
    if serializer.is_valid():
        serializer.save()
...
def user_detail(request, user_id):
    ...
    return Response(UserS(user, context={'request': request}).data)
...
def list_friends(request):
    ...
    return Response(UserS(friends, many=True).data)
...
def search_users(request):
    ...
    return Response(UserS(users, many=True).data)
